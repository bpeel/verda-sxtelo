...
{
  VsxFlipTileHandler *handler = object;
...
{
  VsxFlipTileHandler *self = (VsxFlipTileHandler *) handler;
  VsxPersonId id;

  if (method == VSX_REQUEST_METHOD_GET
      && vsx_arguments_parse ("pi", query_string, &id, &self->tile_num)
  ...
  if (self->person)
    {
      if (self->person->conversation)
        vsx_conversation_flip_tile (self->person->conversation,
                                    self->tile_num);
...
vsx_flip_tile_handler_get_class (void)
{
  ...
  if (klass.parent_class.free == NULL)
    {
      ...
      klass.parent_class.instance_size = sizeof (VsxFlipTileHandler);
...
vsx_flip_tile_handler_new (void)
{
  VsxRequestHandler *handler =
    vsx_object_allocate (vsx_flip_tile_handler_get_class ());
