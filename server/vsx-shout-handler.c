...
{
  VsxShoutHandler *handler = object;
...
{
  VsxShoutHandler *self = (VsxShoutHandler *) handler;
  VsxPersonId id;
  ...
  if (self->person)
    {
      if (self->person->conversation)
        vsx_conversation_shout (self->person->conversation,
...
vsx_shout_handler_get_class (void)
{
  ...
  if (klass.parent_class.free == NULL)
    {
      ...
      klass.parent_class.instance_size = sizeof (VsxShoutHandler);
...
VsxRequestHandler *
vsx_shout_handler_new (void)
{
  VsxRequestHandler *handler =
    vsx_object_allocate (vsx_shout_handler_get_class ());
