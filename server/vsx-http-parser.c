...
{
  parser->buf_len = 0;
  parser->state = VSX_HTTP_PARSER_READING_REQUEST_LINE;
...
add_bytes_to_buffer (VsxHttpParser *parser,
                     ...
                     GError **error)
{
  if (parser->buf_len + length > VSX_HTTP_PARSER_MAX_LINE_LENGTH)
    {
      g_set_error (error,
                   VSX_HTTP_PARSER_ERROR,
                   VSX_HTTP_PARSER_ERROR_UNSUPPORTED,
                      ...
                      GError **error)
{
  ...
  *uri_end = '\0';
  ...
  /* Assume there is no data unless we get a header specifying
     otherwise */
  parser->transfer_encoding = VSX_HTTP_PARSER_TRANSFER_NONE;
  ...
  if (!g_ascii_strcasecmp (field_name, "content-length"))
    {
      ...
      parser->transfer_encoding = VSX_HTTP_PARSER_TRANSFER_CONTENT_LENGTH;
    }
  else if (!g_ascii_strcasecmp (field_name, "transfer-encoding"))
    {
      ...
      parser->transfer_encoding = VSX_HTTP_PARSER_TRANSFER_CHUNKED;
    }
...
static gboolean
process_request_finished (VsxHttpParser *parser,
                          GError **error)
{
  ...
  while (length > 0)
    {
      switch (parser->state)
        {
        case VSX_HTTP_PARSER_READING_REQUEST_LINE:
          ...
          if ((terminator = memchr (data, '\r', length)))
            {
              ...
              parser->state = VSX_HTTP_PARSER_TERMINATING_REQUEST_LINE;
            }
          else
            {
        ...
        case VSX_HTTP_PARSER_TERMINATING_REQUEST_LINE:
          ...
          if (data[0] == '\n')
            {
              ...
              if (parser->buf_len == 0)
              ...
              else
                {
                  ...
                  parser->state = VSX_HTTP_PARSER_READING_HEADER;
                }
        ...
        case VSX_HTTP_PARSER_READING_HEADER:
          /* Could the data contain a terminator? */
          if ((terminator = memchr (data, '\r', length)))
            {
              /* Add the data up to the potential terminator */
              ...
              parser->state = VSX_HTTP_PARSER_TERMINATING_HEADER;
            }
          else
            {
              /* Add and consume all of the data */
        ...
        case VSX_HTTP_PARSER_TERMINATING_HEADER:
          /* Do we have the \n needed to complete the terminator? */
          if (data[0] == '\n')
            {
              ...
              if (parser->buf_len == 0)
                switch (parser->transfer_encoding)
                  {
                  case VSX_HTTP_PARSER_TRANSFER_NONE:
                  ...
                  case VSX_HTTP_PARSER_TRANSFER_CONTENT_LENGTH:
                    parser->state = VSX_HTTP_PARSER_READING_DATA_WITH_LENGTH;
                    break;

                  case VSX_HTTP_PARSER_TRANSFER_CHUNKED:
                    parser->state = VSX_HTTP_PARSER_READING_CHUNK_LENGTH;
                    parser->content_length = 0;
                    break;
                  }
              else
                /* Start checking for a continuation */
                parser->state = VSX_HTTP_PARSER_CHECKING_HEADER_CONTINUATION;
        ...
        case VSX_HTTP_PARSER_CHECKING_HEADER_CONTINUATION:
        ...
        case VSX_HTTP_PARSER_READING_DATA_WITH_LENGTH:
          {
        ...
        case VSX_HTTP_PARSER_READING_CHUNK_LENGTH:
          if (g_ascii_isxdigit (*data))
            ...
            parser->state = VSX_HTTP_PARSER_IGNORING_CHUNK_EXTENSION;
          else if (*data == '\r')
            {
              data++;
              length--;
              parser->state = VSX_HTTP_PARSER_TERMINATING_CHUNK_LENGTH;
            }
          else
            {
              g_set_error (error,
        ...
        case VSX_HTTP_PARSER_TERMINATING_CHUNK_LENGTH:
          if (*data != '\n')
            {
              g_set_error (error,
                           VSX_HTTP_PARSER_ERROR,
          ...
          if (parser->content_length)
            parser->state = VSX_HTTP_PARSER_READING_CHUNK;
          else
            parser->state = VSX_HTTP_PARSER_IGNORING_CHUNK_TRAILER;
          break;

        case VSX_HTTP_PARSER_IGNORING_CHUNK_EXTENSION:
          /* Could the data contain a terminator? */
          if ((terminator = memchr (data, '\r', length)))
            {
              parser->state = VSX_HTTP_PARSER_TERMINATING_CHUNK_EXTENSION;
              length -= terminator - data + 1;
              data = terminator + 1;
            }
          else
        ...
        case VSX_HTTP_PARSER_TERMINATING_CHUNK_EXTENSION:
          if (*data == '\n')
            {
              data++;
              length--;
        ...
        case VSX_HTTP_PARSER_IGNORING_CHUNK_TRAILER:
          /* Could the data contain a terminator? */
          if ((terminator = memchr (data, '\r', length)))
            {
              parser->state = VSX_HTTP_PARSER_TERMINATING_CHUNK_TRAILER;
        ...
        case VSX_HTTP_PARSER_TERMINATING_CHUNK_TRAILER:
          if (*data == '\n')
            {
              length--;
              data++;
        ...
        case VSX_HTTP_PARSER_READING_CHUNK:
          {
            unsigned int to_process_length = MIN (parser->content_length,
                                                  length);
            ...
            if (parser->content_length == 0)
              ...
              parser->state = VSX_HTTP_PARSER_READING_CHUNK_TERMINATOR1;
          }
          break;

        case VSX_HTTP_PARSER_READING_CHUNK_TERMINATOR1:
          {
            if (*data != '\r')
              {
                g_set_error (error,
            ...
            parser->state = VSX_HTTP_PARSER_READING_CHUNK_TERMINATOR2;
          }
          break;

        case VSX_HTTP_PARSER_READING_CHUNK_TERMINATOR2:
          {
            if (*data != '\n')
              {
                g_set_error (error,
...
{
  return g_quark_from_static_string ("vsx-http-parser-error");
}
