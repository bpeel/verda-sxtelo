...
{
  GmlLeaveHandler *handler = GML_LEAVE_HANDLER (object);
  ...
  G_OBJECT_CLASS (gml_leave_handler_parent_class)->dispose (object);
}
...
{
  GmlLeaveHandler *self = GML_LEAVE_HANDLER (handler);
  GmlPersonId id;
...
static void
gml_leave_handler_class_init (GmlLeaveHandlerClass *klass)
{
...
static void
gml_leave_handler_init (GmlLeaveHandler *self)
{
}
